...
# Integrated Shiny App: CHIRPS (3 existing tabs) + Tab4 (CHIRTS daily) + Tab5 (CHIRP v3)
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          numericInput("start_year_daily", "Start Year:", value = 2025, min = 1981, max = 2050),
          ...
          numericInput("end_year_daily", "End Year:", value = 2025, min = 1981, max = 2050),
...
server <- function(input, output, session) {
  ...
  # (unchanged, but implemented here)
  # ---------------------------
  ...
  observeEvent(input$download_pent, {
    ...
    withProgress(message="Downloading CHIRPS data...", value=0, {
      ...
      for(i in seq_along(files)) {
        ...
        if (action=="download") {
          success <- tryCatch({
            download.file(files[[fname]], destfile, mode="wb", quiet=TRUE)
            TRUE
          }, error=function(e){
  ...
  # (unchanged)
  # ---------------------------
  base_url_daily <- "https://data.chc.ucsb.edu/products/CHIRPS/v3.0/daily/final/ERA5/"
  ...
  observeEvent(input$download_daily, {
    ...
    while (is_before_or_equal(cur$year, cur$month, cur$day, end$year, end$month, end$day)) {
      fname <- sprintf("chirps-v3.0.%04d.%02d.%02d.tif", cur$year, cur$month, cur$day)
    ...
    withProgress(message="Downloading CHIRPS daily files...", value=0, {
      ...
      for (i in seq_along(files)) {
        ...
        if (action=="download") {
          success <- tryCatch({ download.file(url,destfile,mode="wb",quiet=TRUE); TRUE },
  ...
  # Keep UI labels/messages identical to the separate script; inputs are prefixed with tab4_
  # ---------------------------
  ...
  # Keep UI labels/messages identical to the separate script; inputs prefixed with tab5_
  # ---------------------------
